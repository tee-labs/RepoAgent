...
在计算机编程领域，全面的项目文档的重要性，包括每个源代码文件的详细解释，不言而喻。这样的文档是理解、维护和增强代码库的基石。它提供了代码的必要上下文和理由，使当前和未来的开发者更容易理解软件的目的、功能和结构。它不仅便于当前和未来的开发者理解项目的目的和结构，还确保了项目随时间的推移保持可访问和可修改，大大简化了新团队成员的学习曲线。
...
- **🌐 多语言支持，基于 [codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp) — 支持 Python、Java、Go、TypeScript、C/C++ 等 150+ 种语言（tree-sitter）。**
- **🤖 自动检测Git仓库中的变化，跟踪文件的增加、删除和修改。**
- **📝 独立分析代码结构，为各个对象（函数、类、方法）生成文档。**
...
RepoAgent 使用 [codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp) 作为代码智能后端，用于 AST 解析和交叉引用分析。`codebase-memory-mcp` 二进制文件会在首次运行时自动下载（通过 `codebase-memory-mcp` PyPI 包），无需手动安装。
...
repoagent run --print-hierarchy # 此命令将打印repoagent解析出的目标仓库
```
...
- `-m`, `--model` TEXT：指定用于完成的模型。默认值：`gpt-4o-mini`
...
- `-fe`, `--file-extensions` TEXT：要处理的源代码文件扩展名列表（不含点号），用逗号分隔。例如：`py` 表示 Python，`java` 表示 Java，`py,java` 表示同时处理两种语言。默认值：`py`
...
repos:
  - repo: local
    ...
    - id: repo-agent
      ...
      # 钩子触发的文件类型（支持 codebase-memory-mcp 的任意语言）
      types: [python]
      # Java 项目请使用: types: [java]
      # 多语言项目可添加多个 hook 条目，或使用 types: [file]
```
...
- [x] **多编程语言支持** — 基于 [codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)（tree-sitter），现已支持 Python、Java、Go、TypeScript、C/C++ 等 150+ 种语言
- [x] 本地模型支持如 Llama、chatGLM、Qianwen 等
